...
spec = do
  describe "Unit test" $ do
    it "check a typeless statement" $
      parse "a = 1" `shouldBe` Right ([TypeDecl (AlexPn 2 1 3) (ModifiedType [] None) (Ident "a"), VarDecl (AlexPn 2 1 3) (Ident "a") (Imm' 1)])
    ...
    it "check multiple statements" $
      parse "double :: x = 1.0\ny = x" `shouldBe` Right [TypeDecl (AlexPn 7 1 8) (ModifiedType [] $ Ident "double") (Ident "x"),VarDecl (AlexPn 12 1 13) (Ident "x") (Imm' 1.0), TypeDecl (AlexPn 20 2 3) (ModifiedType [] None) (Ident "y"), VarDecl (AlexPn 20 2 3) (Ident "y") (Ident' "x")]
    ...
    it "check arithmetic expressions" $
      parse "y = -4*x**2 + a*x - 3/2\nz = +(a,b) + (3*(1,2))" `shouldBe`
        Right [ TypeDecl (AlexPn 2 1 3) (ModifiedType [] None) (Ident "y")
              , VarDecl (AlexPn 2 1 3) (Ident "y")
                                       (Binop' Sub (Binop' Add (Binop' Mul (Uniop' Minus (Imm' 4)) (Binop' Pow (Ident' "x") (Imm' 2)))
                                                               (Binop' Mul (Ident' "a") (Ident' "x")))
                                                   (Binop' Div (Imm' 3) (Imm' 2)))
              , TypeDecl (AlexPn 26 2 3) (ModifiedType [] None) (Ident "z")
    ...
    it "check let-in expression" $
      parse "y = let a = 1\nb=2\nin a**2 + b**2" `shouldBe`
        Right [ TypeDecl (AlexPn 2 1 3) (ModifiedType [] None) (Ident "y")
              , VarDecl (AlexPn 2 1 3) (Ident "y")
                                       (Let' [TypeDecl (AlexPn 10 1 11) (ModifiedType [] None) (Ident "a"), VarDecl (AlexPn 10 1 11) (Ident "a") (Imm' 1)
                                             ,TypeDecl (AlexPn 15 2 2) (ModifiedType [] None) (Ident "b"), VarDecl (AlexPn 15 2 2) (Ident "b") (Imm' 2)]
    ...
    it "check if expression" $
      parse "x = if a then b else c + d" `shouldBe`
        Right [ TypeDecl (AlexPn 2 1 3) (ModifiedType [] None) (Ident "x")
              , VarDecl (AlexPn 2 1 3) (Ident "x") (If' (Ident' "a") (Ident' "b") (Binop' Add (Ident' "c") (Ident' "d")))
              ]
    it "check function application" $
      parse "y = f x\nz = g(1.0,a)\nw = (f.g)(1.0,a)" `shouldBe`
        Right [ TypeDecl (AlexPn 2 1 3) (ModifiedType [] None) (Ident "y")
              , VarDecl (AlexPn 2 1 3) (Ident "y") (App' (Ident' "f") (Ident' "x"))
    ...
    it "check comment" $ do
      parse "z = 3\n\n## hoge\n#foo\ndouble :: x = 1 # bar\ny = 2" `shouldBe`
        Right [ TypeDecl (AlexPn 2 1 3) (ModifiedType [] None) (Ident "z")
              , VarDecl (AlexPn 2 1 3) (Ident "z") (Imm' 3)
              , TypeDecl (AlexPn 27 5 8) (ModifiedType [] (Ident "double")) (Ident "x")
              , VarDecl (AlexPn 32 5 13) (Ident "x") (Imm' 1)
              , TypeDecl (AlexPn 44 6 3) (ModifiedType [] None) (Ident "y")
              , VarDecl (AlexPn 44 6 3) (Ident "y") (Imm' 2)
              ]
    it "check empty statement" $
      parse "x = 1\n\n\ny = 2" `shouldBe`
        Right [ TypeDecl (AlexPn 2 1 3) (ModifiedType [] None) (Ident "x")
              , VarDecl (AlexPn 2 1 3) (Ident "x") (Imm' 1)
              , TypeDecl (AlexPn 10 4 3) (ModifiedType [] None) (Ident "y")
              , VarDecl (AlexPn 10 4 3) (Ident "y") (Imm' 2)
              ]
    it "check tuples" $
      parse "y = (x)\nz = (a,b)" `shouldBe`
        Right [ TypeDecl (AlexPn 2 1 3) (ModifiedType [] None) (Ident "y")
              , VarDecl (AlexPn 2 1 3) (Ident "y") (Ident' "x")
              , TypeDecl (AlexPn 10 2 3) (ModifiedType [] None) (Ident "z")
